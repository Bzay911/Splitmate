...
export const ExpenseProvider = ({ children }: ExpenseProviderProps) => {
  ...
  const groupTotalExpense = expenses.reduce((sum, expense) => sum + expense.amount, 0); 
  const groupTotalMembers = groupMembers.length;
  const fairshare = groupTotalExpense / groupTotalMembers;

  // Calculate settlement adjustments for each member
  const getSettlementAdjustments = useCallback((memberId: string) => {
    let adjustments = 0;
  ...
  // Calculate balances for each member (including settlements)
  useEffect(() => {
    if (groupMembers.length > 0 && expenses.length > 0 && fairshare > 0) {
      const newBalances = groupMembers.map((member) => {
        const individualExpense = getIndividualExpense(member);
        const baseBalance = individualExpense - fairshare;
  ...
  }, [groupMembers, expenses, fairshare, getIndividualExpense, settlements, getSettlementAdjustments]);
  ...
  const addSettlementToState = (settlementData: any) => {
    ...
    const newSettlement: BackendSettlement = {
      ...
      fromUser: {
        _id: settlementData.fromUserId || getUserIdFromDisplayName(settlementData.fromUser?.displayName) || '',
        displayName: settlementData.fromUser?.displayName || '',
        email: settlementData.fromUser?.email || ''
      },
      toUser: {
        _id: settlementData.toUserId || getUserIdFromDisplayName(settlementData.toUser?.displayName) || '',
  ...
  // console.log("whoNeedsToPayWhom", whoNeedsToPayWhom());
